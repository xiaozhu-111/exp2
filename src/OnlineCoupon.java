...
//Mapper 处理的主要是 value(每行的文本内容)
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.err.println("Usage: OnlineCouponAnalysis <input> <output>");
            System.exit(2);
        }

        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf, "Online Coupon Analysis");

        job.setJarByClass(OnlineCoupon.class);
